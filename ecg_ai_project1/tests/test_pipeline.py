...
def test_preprocess_ecg_handles_signal_shorter_than_baseline_window():
    # Regression test: a signal shorter than remove_baseline_wander's
    # smoothing window (0.6s * fs) used to crash with a numpy broadcast
    # error, since np.convolve(..., mode="same") returns the *longer* of
    # the signal/kernel lengths rather than always matching the signal.
    # Reproduces a real crash from a tightly-cropped digitized ECG image
    # (~0.4s of signal at 500Hz).
    ecg = generate_synthetic_ecg(SyntheticECGParams(), fs=500, duration_s=10, seed=0)[:, :205]
    out = preprocess_ecg(ecg, orig_fs=500, target_fs=500, target_seconds=205 / 500)
    assert out.shape == (NUM_LEADS, 205)
